...
class TobiiProTracker(BaseEyeTracker):
    ...
    def __init__(self, display, logfile=settings.LOGFILE,
        ...
        self.datafile = open("{0}_TOBII_output.tsv".format(logfile), 'a')

        # CUSTOMIZING BY JSRPIRIT (in order to save gaze output data into participant folder)
    ...
    def initiation_report(self, logfile):
        # CUSTOMIZING BY JSRPIRIT (in order to save gaze output data into participant folder)
        print("{0}/TOBII_output.tsv".format(logfile))
        self.datafile = open("{0}/TOBII_output.tsv".format(logfile), 'a')
    ...
    def calibrate(self, calibrate=True, validate=True):
        ...
        if calibrate:
            ...
            while calibrating:
                ...
                for point in self.points_to_calibrate:
                    pressed_key = self.kb.get_key(keylist=['escape', 'q'])
                    if pressed_key[0] == 'escape' or pressed_key[0] == 'q':
                ...
                pressed_key = self.kb.get_key(keylist=['space', 'r', 'escape', 'q'], flush=True, timeout=None)
                if pressed_key[0] == 'escape' or pressed_key[0] == 'q':
                    return False
        ...
        if validate:
            ...
            pressed_key = self.kb.get_key(keylist=['space', 'escape', 'q'], flush=True, timeout=None)
            if pressed_key[0] == 'escape' or pressed_key[0] == 'q':
                return False
            if pressed_key[0] == 'space':
                pass
    ...
    def wait_for_fixation_start(self):
        ...
        # get starting time
        t0 = clock.get_time()
        print("wait_for_fixation_start: ", t0)
        ...
        while moving:
            ...
            if self.is_valid_sample(npos):
                ...
                if (npos[0] - spos[0])**2 + (npos[1] - spos[1])**2 > self.pxfixtresh**2:  # Pythagoras
                    ...
                    spos = copy.copy(npos)
                    t0 = clock.get_time()
                    print("wait_for_fixation_start (while): ", t0)
    ...
    def wait_for_saccade_start(self):
        ...
        print("wait_for_saccade_start: ", t0)
    ...
    def wait_for_saccade_end(self):
        ...
        t1 = clock.get_time()
        print("wait_for_saccade_end (t1): ", t1)
    ...
    def log(self, msg):
        ...
        # ts = time.time()
        self.datafile.write("{}\t{}\t{}\n".format(round((t - self.t0) / 1000.0, ndigits=4),ts, msg))
